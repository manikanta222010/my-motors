...
// useParams to get id from link of the bike
  const { id } = useParams();

// useState on bikes to take care of updation of that variable 
  ...
  return bike ? <UpdateBike bike={bike} /> : "";
}
...
// useHistory to change page link(Navigation purpose)
  ...
  const [modal, setModal] = useState(bike.modal);
  const [description, setDescription] = useState(bike.description);
  const [engine, setEngine] = useState(bike.engine);
  const [price, setPrice] = useState(bike.price);
  const [url, setUrl] = useState(bike.url);

  // Function to edit bike in existing data
  const editBike = () => {
    const updatedBike = {
      brand, modal, description, engine, price, url
    };

    fetch(`https://619a40d09022ea0017a7b0a9.mockapi.io/bikes/${bike.id}`,
      {
        method: "PUT",
...
// useFormik used for validation of the form used to add bike
  const formik = useFormik({
    initialValues: { brand: brand, modal: modal, description: description, engine: engine, price: price, url: url },
